...
function header_sticky() {
    ...
    var header = jQuery("header.transparent");
    var container = header.find(".container");
    var mainmenu = header.find(".main-menu");

    // Hilangkan class jika lebar layar di bawah 992px
    if (window.innerWidth < 992) {
        container.removeClass("mt-3");
        mainmenu.removeClass("bg-blur");
        return; // Tidak lanjutkan scroll logic saat mobile
    }

    if ($document.scrollTop() >= 0.1) {
        header.removeClass("scrollOff").addClass("scrollOn").css("height", "auto");
        mainmenu.removeClass("bg-blur");
        container.removeClass("mt-3");
    } else {
        header.removeClass("scrollOn").addClass("scrollOff");
        mainmenu.addClass("bg-blur");
        container.addClass("mt-3");
    }
}

// Jalankan saat scroll
$(window).on("scroll", header_sticky);
...
// Jalankan juga saat ukuran layar berubah (resizing)
